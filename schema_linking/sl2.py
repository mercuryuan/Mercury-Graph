...
{{
  ...
  "to_solve_the_question": {{
    ...
    "reason": "<why this question is solvable/un-solvable>"
  }}
}}
```
...
def select_relevant_tables(db_schema: str, question: str, select_table: List[str], result_from_last_round='') -> Dict:
...
def generate_schema_description(selected_table):
...
def generate_result_from_last_round(result: str):
    """
    生成上一轮的结果,作为提示的一部分
    :param result:
    :return:
...
if __name__ == '__main__':
    ...
    selected_table = ["actor", "cast"]
    db_schema = generate_schema_description(selected_table)
    ...
    print("Final Prompt:\n", final_prompt)
    # result = select_relevant_tables(db_schema, question, selected_table)
