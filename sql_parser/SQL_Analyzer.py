...
class SQLAnalyzer:
    ...
    def analyze_sql_query(self, entry, output_mode="pass_basic_fail_full",name_correction=True):
        ...
        is_valid_in_neo4j = self.tool.display_parsing_result(sql, question, db_id,
                                                             output_mode,name_correction)
        return is_valid_in_neo4j

    def analyze_sql_data(self, data_list, output_mode,name_correction):
        ...
        try:
            for index, entry in enumerate(data_list, start=1):
                ...
                try:
                    if self.analyze_sql_query(entry, output_mode,name_correction):
                        passed_count += 1
            ...
            self.tool.log(summary, "./analysis_summaries_without_correction.log")

    def analyze_sql_by_database(self, output_mode,name_correction):
        """
        通过数据库名称筛选数据并分析 SQL。
        ...
        self.analyze_sql_data(data, output_mode,name_correction)


if __name__ == "__main__":
    # analyzer = SQLAnalyzer("bird", "books")
    # analyzer.analyze_sql_by_database(output_mode="pass_basic_fail_full")
    # 遍历分析spider所有数据库
    dataset = "spider"
    for database in DataLoader(dataset).list_dbname(show=False):
        ...
        analyzer.analyze_sql_by_database(output_mode="pass_silent_fail_full",name_correction=False)
    # 遍历分析 bird 所有数据库
    dataset = "bird"
    for database in DataLoader(dataset).list_dbname(show=False):
        analyzer = SQLAnalyzer(dataset, database)
